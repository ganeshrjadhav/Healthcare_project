...
df3 = df2.withColumn('Facility_Address',
                     concat_ws(',', 'Address Line 1', 'Address Line 2', 'County/Parish', 'ZIP Code')) \
    .drop('Address Line 1') \
    .drop('Address Line 2') \
    .drop('ZIP Code') \
    .drop('County/Parish')
...
df5 = df4.drop('Five Star Date', 'Hgb > 12 data availability code', 'Number of Dialysis Patients with Hgb data',
               'Patient Transfusion category text', 'Number of patients included in the transfusion summary') \
...
df5 = df5.drop('Number of adult HD patients with KT/V data', 'Number of adult PD patients with KT/V data',
               'Number of pediatric HD patients with Kt/V data',
               'Number of patient-months in hypercalcemia summary', 'SRR Date', 'Patient hospitalization category text',
               'Patient Hospital Readmission Category', 'Mortality Rate: Upper Confidence Limit (97.5%)',
               'Readmission Rate: Upper Confidence Limit (97.5%)', 'Readmission Rate: Lower Confidence Limit (2.5%)',
               'Hospitalization Rate: Upper Confidence Limit (97.5%)',
               'Readmission Rate: Lower Confidence Limit (2.5%)',
               'Hospitalization Rate: Upper Confidence Limit (97.5%)',
               'Hospitalization Rate: Lower Confidence Limit (2.5%)', 'SIR Date', 'Patient Infection category text',
               'SIR: Upper Confidence Limit (97.5%)', 'SIR: Upper Confidence Limit (97.5%)',
               'SIR: Lower Confidence Limit (2.5%)', 'Transfusion Rate: Upper Confidence Limit (97.5%)',
               'Transfusion Rate: Lower Confidence Limit (2.5%)', 'Fistula Category Text',
               'Fistula Rate: Upper Confidence Limit (97.5%)', 'Fistula Rate: Lower Confidence Limit (2.5%)',
               'Long term catheter Data Availability Code', 'Number of patients in nPCR summary', 'SWR Date',
               'SWR category text', 'Patient transplant waitlist data availability code',
               '95% C.I. (upper limit) for SWR', '95% C.I. (lower limit) for SWR', 'PPPW category text',
    ...
    .withColumnRenamed('Percentage of Adult patients with hypercalcemia (serum calcium greater than 10.2 mg/dL)',
                       '%_Patients_with_Hypercalcemia_(>10.2 mg/dL)') \
    .withColumnRenamed('Percentage of Adult PD PTS with Kt/V >= 1.7', '%_Adult_PD_patients_Hgb<10 g/dL') \
    .withColumnRenamed('Number of patients in Serum phosphorus summary', 'Patients_in_Serum_Phosphorous_summary') \
    .withColumnRenamed('Percentage of Adult patients with serum phosphorus less than 3.5 mg/dL',
                       '%_Adult_Patients_with_serum_phosph._<3.5mg/dL') \
    .withColumnRenamed('Percentage of Adult patients with serum phosphorus between 3.5-4.5 mg/dL',
                       '%_Adult_Patients_with_serum_phosph._btw_3.5-4.5 mg/dL') \
    .withColumnRenamed('Percentage of Adult patients with serum phosphorus between 4.6-5.5 mg/dL',
                       '%_Adult_Patients_with_serum_phosph._btw_4.6-5.5 mg/dL mg/dL') \
    .withColumnRenamed('Percentage of Adult patients with serum phosphorus between 5.6-7.0 mg/dL',
                       '%_Adult_Patients_with_serum_phosph._btw_5.6-7.0 mg/dL') \
    .withColumnRenamed('Percentage of Adult patients with serum phosphorus greater than 7.0 mg/dL',
                       '%_Adult_Patients_with_serum_phosph._>7.0mg/dL') \
    .withColumnRenamed('Number of patients included in hospitalization summary',
                       'Num_Patients_in_hospitalization_summary') \
    .withColumnRenamed('Number of hospitalizations included in hospital readmission summary',
                       'Num_Patients_in_hospitalization_readmission_summary') \
    ...
    .withColumnRenamed('Number of patient months in long term catheter summary',
                       'Num_Patients_months_in_long_term_catheter_summary') \
    .withColumnRenamed('Percentage of Adult patients with long term catheter in use',
                       '%_Adult_patients_with_long_term_catheter') \
...
df7 = df6.fillna(value='0', subset=['Facility rating'])

df7.display()

# COMMAND ----------

df7.select('Facility rating').display()

# COMMAND ----------

df7.fillna.('0', ['Facility rating']).display()

# COMMAND ----------

# convert the Facility Name and Chain Organization data into uppercase

from pyspark.sql.functions import upper, col

df7 = df7.withColumn('Facility Name', upper(col('Facility Name'))) \
    .withColumn('Chain Organization', upper(col('Chain Organization'))) \
 \
    df7.display()

# COMMAND ----------

# converting all column names into uppercase
for col in df7.columns:
    df7 = df7.withColumnRenamed(col, col.upper())

df7.display()

# COMMAND ----------

# splitting date column into start date and end date
from pyspark.sql.functions import split

df7 = df7.withColumn('CLAIMS_START_DATE', split(df7['CLAIMS DATE'], '-').getItem(0)) \
    .withColumn('CLAIMS_END_DATE', split(df7['CLAIMS DATE'], '-').getItem(1)) \
 \
    df7.display()

# COMMAND ----------

# converting dates into pyspark readable format

from pyspark.sql.functions import col, udf
from pyspark.sql.types import StringType


def convert_date(strn):
    months = {'JAN': '01', 'FEB': '02', 'MAR': '03', 'JUL': '07', 'APR': '04', 'MAY': '05', 'JUN': '06', 'AUG': '08',
...
udf_con_date = udf(lambda x: convert_date(x), StringType())

df8 = df7.withColumn('CLAIMS_START_DATE1', udf_con_date(col('CLAIMS_START_DATE'))) \
    .withColumn('CLAIMS_END_DATE1', udf_con_date(col('CLAIMS_END_DATE'))) \
 \
    df8 = df8.withColumn('test', col('CLAIMS_START_DATE'))
df8.display()

# COMMAND ----------

# drop redundatnt columns

df9 = df8.drop('test', 'CLAIMS_START_DATE', 'CLAIMS_END_DATE')
